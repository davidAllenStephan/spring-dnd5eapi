...
@FeignClient(name = "dnd5eapi-backgrounds", url = "https://www.dnd5eapi.co/api/2014")
@Service
public interface Dnd5eApiBackgrounds {
    @GetMapping("/backgrounds")
    Dnd5eApiNamedResourceList getBackgrounds();

    @GetMapping("/backgrounds/{index}")
    @Cacheable(value = "cache", key = "#root.methodName+#root.args")
    Dnd5eApiBackground getBackgroundByIndex(@PathVariable String index);
}
