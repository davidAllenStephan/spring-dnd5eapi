...
@FeignClient(name = "dnd5eapi-conditions", url = "https://www.dnd5eapi.co/api/2014")
@Service
public interface Dnd5eApiConditions {
    @GetMapping("/conditions")
    Dnd5eApiNamedResourceList getConditions();

    @GetMapping("/conditions/{index}")
    @Cacheable(value = "cache", key = "#root.methodName+#root.args")
    Dnd5eApiCondition getConditionByIndex(@PathVariable("index") String index);
}
