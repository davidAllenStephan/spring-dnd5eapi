...
@FeignClient(name = "dnd5eapi-proficiencies", url = "https://www.dnd5eapi.co/api/2014")
@Service
public interface Dnd5eApiProficiencies {
    @GetMapping("/proficiencies")
    Dnd5eApiNamedResourceList getProficiencies();

    @GetMapping("/proficiencies/{index}")
    @Cacheable(value = "cache", key = "#root.methodName+#root.args")
    Dnd5eApiProficiency getProficiencyByIndex(@PathVariable("index") String index);
}
