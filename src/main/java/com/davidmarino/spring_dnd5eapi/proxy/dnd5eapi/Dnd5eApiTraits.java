...
@FeignClient(name = "dnd5eapi-traits", url = "https://www.dnd5eapi.co/api/2014")
@Service
public interface Dnd5eApiTraits {
    @GetMapping("/traits")
    Dnd5eApiNamedResourceList getTraits();

    @GetMapping("/traits/{index}")
    @Cacheable(value = "cache", key = "#root.methodName+#root.args")
    Dnd5eApiTrait getTraitByIndex(@PathVariable String index);
}
